...
for ii=1:numel(Sess_MRI_ID)
    ...
    if exist(cell2char(dir_path_in),'dir')==0 %0 means DNE!
        ...
        if ~exist(nii_outgz)
            ...
            cmds(cc_cmds)={[ 'if [ -f  ' nii_out '  ] ; then rm ' nii_out ' ; fi \n'   ]};
            cc_cmds=cc_cmds+1;
        ...
        %Write only if nii_outgz doesn't exist, so something is being done!
        if ~exist(nii_outgz)
            %Copying th ectual niftii file:
            cmds(cc_cmds)={[ '### Beg of ' mfilename '.m in' cell2char(Sess_MRI_ID(ii)) '\n'] };
            echoer={['echo  Copying session: ' cell2char(Sess_MRI_ID(ii)) '...\n']};
            cmds= [echoer cmds];
            cc_cmds=cc_cmds+1;
            
            %Space to separate
            cmds(cc_cmds)={[ '### End of ' mfilename '.m in ' cell2char(Sess_MRI_ID(ii)) '###\n \n \n \n'] };
            cc_cmds=cc_cmds+1;
            cmds(cc_cmds)={ '\n' };
